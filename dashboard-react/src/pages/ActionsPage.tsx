...
import { PerformanceGrid } from '../components/Actions/PerformanceGrid';
...
export function ActionsPage({ data, matchAction }: { data: DashboardData; matchAction: (a: { search_term?: string; experiment_id?: string; net_roas?: number; cpc?: number; conv_rate?: number }) => { gt: GroundTruth; supported: boolean }[] }) {
  ...
  // Current sellable stock per ASIN — the OOS guard parks negates/promotes on empty shelves.
  const sellableByAsin = useMemo(() => {
    const m = new Map<string, number>();
    for (const r of data.supply_chain || []) if (r.asin) m.set(r.asin, r.sellable_qty ?? 0);
    return m;
  }, [data.supply_chain]);
            ...
            {budgetRows.map(a => {
              ...
              return (
                <div key={`${a.campaign_id}:${a.action}`} className="rounded-lg border border-[var(--color-border)] bg-[var(--color-card)] p-2.5">
                  ...
                  <div className="my-1"><PerformanceGrid action={a} /></div>
