...
SELECT 
  Date,
  ASIN,
  SUM(`Ending Warehouse Balance`) AS quantity_balance,
  'FBA' AS source_type
...
SELECT 
  Date,
  ASIN,
  -- AWD is per package, convert to units by multiplying by Package Quantity
  -- Package Quantity represents units per package
  SUM(`Ending Warehouse Balance` * COALESCE(`Package Quantity`, 1)) AS quantity_balance,
  'AWD' AS source_type
