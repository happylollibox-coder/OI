...
for fam in top_families:
    ...
    for phase in ['OFF_SEASON', 'PEAK']:
        ...
        for _, r in top.iterrows():
            st = str(r['search_term']).replace("asin=", "")[:25].ljust(25)
            output += f"  > {st} | NetPrf: ${r['net_profit']:,.0f} | CPC: ${r['cpc']:.2f} | ROAS: {r['net_roas']:.1f}x | Orders: {r['orders']:.0f}\n"
        output += "\n"

with open('.tmp/family_summary_filtered.txt', 'w') as f:
    f.write(output)
print("Done.")
