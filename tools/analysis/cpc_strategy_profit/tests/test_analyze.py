...
def _cell(rows):
    cols = ["parent_name", "calendar_segment", "cpc_action",
            "n_regimes", "clicks", "orders", "net_profit", "net_profit_per_day", "verdict"]
    return pd.DataFrame(rows, columns=cols)

def test_rank_picks_highest_npd_among_conclusive():
    cells = _cell([
        ["Bottle", "EVERYDAY_2026-01", "RAISE",    5, 999, 99, 100.0, 9.0, "CONCLUSIVE"],
        ["Bottle", "EVERYDAY_2026-01", "CONSTANT", 5, 999, 99, 100.0, 4.0, "CONCLUSIVE"],
        ["Bottle", "EVERYDAY_2026-01", "LOWER",    5, 999, 99,  50.0, 12.0, "WEAK"],
    ])
    ranked = rank_strategies(cells)
    top = ranked[(ranked["parent_name"] == "Bottle") & (ranked["rank"] == 1)].iloc[0]
    assert top["cpc_action"] == "RAISE"   # WEAK cell ignored despite higher npd

def test_merge_collapses_segments_with_same_winner():
    ranked = pd.DataFrame([
        ["Bottle", "EVERYDAY_2026-01", "RAISE",    1],
        ["Bottle", "XMAS_PRE",         "RAISE",    1],
        ["Bottle", "XMAS_PEAK",        "CONSTANT", 1],
    ], columns=["parent_name", "calendar_segment", "cpc_action", "rank"])
