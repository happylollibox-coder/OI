...
- **Allocate the product budget (Component 0) to cells → `planned_spend`, by a performance- and season-aware policy:**
  ...
  - **Season scales hard:** a cell's allocation ramps *major* inside its peak window and is **near-zero outside it** — driven by the holiday calendar (`DIM_US_HOLIDAYS` boost/peak windows) + the LY-peak performance ratio. An occasion-dedicated (seasonal) campaign gets ~0 off-season and a major allocation only during its window (e.g. a Christmas campaign funded only at Christmas), reusing the seasonal-campaign boost/cooldown gating ([[feedback_seasonal_terms_dedicated_campaign]]).
