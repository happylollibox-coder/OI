...
**In:** `DE_WEEKLY_PLAN` (cell grain, history-retained, purpose + success_metric + expected_value); `tools/weekly_plan/` generator (purpose assignment + history-calibrated expectations, reads `V_PLAN_LEARNINGS`); `V_WEEKLY_PLAN_REVIEW`; `V_PLAN_LEARNINGS`; `V_WEEKLY_PLAN_ACTIONS`; the lightweight per-product surface; `config.yaml`; tolerance in `DE_COACH_THRESHOLDS`.
