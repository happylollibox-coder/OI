...
**Budget-first, but profit isn't capped.** The plan **starts from a budget** (per product) — but the budget is a **risk guardrail on unproven spend, not a hard cap on profit.** Allocation is performance- and season-aware:
- **Profitable / proven (SCALE) cells → not hard-capped:** they **scale while profitable** (spend up as long as marginal ROAS ≥ target / `net_per_dollar > 0`); their `planned_spend` is a starting floor, and exceeding it is *good* (more profitable volume).
- **Unproven / losing (MAP/PROBE/new/CUT) cells → hard-capped** at a low slice until they prove out — this is where the `weekly_budget` ceiling actually bites (bounding risk).
- **Season scales hard** (per-product peak only — see §4). 

So `weekly_budget` is the **risk envelope** for the unproven portion, with an *optional* manual hard cap for cash-flow/inventory; by default profitable cells run free. Configured Amazon daily budgets ≈ `weekly_budget / 7`, but actual can exceed the configured daily by up to ~2× (Amazon flexes high-traffic days, balancing over the month), so the budget is a **control lever + soft ceiling**, not a hard cap — the review tracks actual-vs-budget and flags overspend early.
...
- **`planned_spend`** + **`spend_mode`** (`SCALE` | `CAP`) — the cell's slice of the product budget (Component 0). `SCALE` (profitable) = `planned_spend` is a floor, spend runs free while profitable; `CAP` (unproven/losing) = hard ceiling at `planned_spend`. Basis for SCALE expected NP.

**Component 0 — the budget (`DE_PRODUCT_BUDGET`).** One editable row per `parent_name × week_start`: `weekly_budget` (the product's spend ceiling), `source` (`MANUAL` | `BUSINESS_PLAN` | `BOOTSTRAP`), `updated_at`. The generator allocates it across the product's active cells by the performance-and-season policy → each cell's `planned_spend`. **Bootstrap default:** when no MANUAL/BUSINESS_PLAN budget exists for a product, seed `weekly_budget` from its **trailing ~8-week actual ad spend** (`source='BOOTSTRAP'`) so the plan is never inert; Ori edits from there. This is the entry point of the whole plan: no budget → no planned spend → no expected results.
...
- **Allocate the product budget (Component 0) to cells → `planned_spend`, by a performance- and season-aware policy:**
  - **Money-makers scale free (not hard-capped):** proven-profitable cells (CONCLUSIVE, `net_per_dollar > 0`) get a `spend_mode='SCALE'` — `planned_spend` is a floor, and they may spend beyond it **while marginal ROAS ≥ target / `net_per_dollar > 0`**. Don't limit profit.
  - **Unproven/losing are hard-capped:** new / MAP / PROBE / margin-losing cells get `spend_mode='CAP'` — a **low fixed slice** that bounds the risk until they start gaining, then the learning loop graduates them to SCALE. The product `weekly_budget` ceiling governs the sum of these CAP cells.
  - **Season scales hard — but peak is performance-defined PER PRODUCT.** An occasion counts as "peak" for a product **only if it actually drives results for that product** (`V_PEAK_RELEVANCE` / the existing per-product peak-relevance + maturity gate), not the blanket calendar. So: an occasion that under-performs for a product (e.g. Prime that doesn't move product X) is **not** treated as peak for X → no peak budget; and an occasion that drives one product hard (e.g. Back-to-School for LolliME) **is** peak for that product → major budget. Where it is peak, the allocation ramps *major* in the window and near-zero outside (a Christmas campaign funded only at Christmas), reusing the seasonal-campaign boost/cooldown gating ([[feedback_seasonal_terms_dedicated_campaign]]) + the LY-peak performance ratio.
  - Sum of cell `planned_spend` ≤ product `weekly_budget`. Translate to configured Amazon daily budgets ≈ `planned_spend / 7` (knowing actual can flex to ~2× daily).
...
- **Budget adherence:** for `CAP` cells, flag `OVERSPEND` early when actual run-rate trends over the cap (Amazon front-loads up to ~2× the configured daily) so it can be pulled back. For `SCALE` cells, spending beyond `planned_spend` is **not** flagged as overspend — it's only a problem if profitability drops (marginal ROAS < target), which surfaces as the SCALE net-profit verdict instead.
- Writes `status` + `actual_value` back onto the reviewed week's `DE_WEEKLY_PLAN` rows (building the corpus). **D/E boundary:** a single OFF_PLAN/MISSED → **D self-corrects at the next weekly boundary** (the generator re-allocates budget / re-assigns purpose / recalibrates `target_cpc` from learnings); **persistent** OFF_PLAN → **E** (escalate to Ori + any mid-week intervention).
...
## 7. Component 5 — surface (DEFERRED to v1.1)

*Not in v1.* Per [[feedback_all_logic_in_backend]], v1 ships the tables/views/tool only; the surface is a pure-presentation layer built next. When built — per product, reusing the **Home Brief** plain-language pattern (no full new page): a "This Week's Plan" card per parent showing, grouped by purpose: the **on/off-plan status vs last week**, **expected vs actual** for each purpose's metric, the **business-plan flag**, the **actions & expected results**, the **strategic/MAP targets**, a compact **forward-3-week** strip, and a **"what we've learned"** line from `V_PLAN_LEARNINGS`. Backed by a Cube loader over the review/actions/learnings views. A dedicated full Plan page is a fast-follow.

## 8. Scope

**In (v1 — BACKEND ONLY, per [[feedback_all_logic_in_backend]]):** `DE_PRODUCT_BUDGET` (per-product weekly ceiling, editable); `DE_WEEKLY_PLAN` (cell grain, history-retained, purpose + success_metric + expected_value + target_cpc + planned_spend); `tools/weekly_plan/` generator (budget allocation → planned_spend, purpose assignment + history-calibrated expectations, reads `V_PLAN_LEARNINGS`); `V_WEEKLY_PLAN_REVIEW`; `V_PLAN_LEARNINGS`; `V_WEEKLY_PLAN_ACTIONS`; `config.yaml`; tolerance in `DE_COACH_THRESHOLDS`. **All decision/allocation logic lives in these views/tool — they are the product.**
**Deferred:** the per-product surface / Cube schema + card (**v1.1** — pure presentation over the v1 views); **mid-week** auto-adjust + escalation on persistent OFF_PLAN (**E**); executing MAP targets / creating campaigns + pushing budgets to Amazon (**F** — D is advisory); a dedicated full Plan page; future-week *action-level* detail.
