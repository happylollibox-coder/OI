...
**Budget-first.** The plan **starts from a budget** (per product, the weekly spend ceiling) before any actions. The budget is **allocated across the product's cells by a performance- and season-aware policy** — **money-makers get more** (proven-profitable cells weighted up by `net_per_dollar`), **unproven/losing get little** (new/MAP/PROBE/margin-losing cells get a low capped slice until they gain), and **season scales hard** (a cell's share ramps major inside its peak window, near-zero outside — e.g. a Christmas campaign funded only at Christmas). That allocation is each cell's **`planned_spend`**, which drives the SCALE expected net profit. Configured Amazon daily budgets ≈ `weekly_budget / 7`, but actual can exceed the configured daily by up to ~2× (Amazon flexes high-traffic days, balancing over the month), so the budget is a **control lever + soft ceiling**, not a hard cap — the review tracks actual-vs-budget and flags overspend early.
...
- **Allocate the product budget (Component 0) to cells → `planned_spend`, by a performance- and season-aware policy:**
  - **Money-makers get more:** proven-profitable cells (CONCLUSIVE, `net_per_dollar > 0`) are weighted *up* by their realized `net_per_dollar` × room-to-grow — the more a cell makes, the larger its share.
  - **Unproven/losing get little:** new / MAP / PROBE / margin-losing cells get a **low capped exploration slice** until they start gaining, then graduate into the money-maker tier (the learning loop flips them).
