...
**Clear-cases presentation (owner, 2026-06-12):** cards are **grouped by family**, families and cards **ordered by weekly dollar opportunity** — `opportunityPerWeek()`: cut = the term's weekly burn (`spend4w/4`); reduce = the weekly loss being stopped (`−netProfit4w/4`); promote = current weekly profit at stake ("scale to beat"). The dollar figure is a **one-week target**: captured on the queue item (`expected_impact_weekly`/`expected_impact_kind`) and — once the gated column lands in `FACT_PPC_CHANGE_LOG` — verified by the outcomes view a week after upload.

**Stage 2 — the receipt loop (action log + scoreboard).**
> **⚠️ Superseded 2026-06-11:** a parallel session shipped this loop as `FACT_PPC_CHANGE_LOG` + `/api/ppc-change-log` + `V_PPC_ACTION_OUTCOMES` (14d pre/post, −2d lag, IMPROVED/WORSE verdicts) + `DecisionScorecard` on the DO page (SOP: `architecture/PPC_CLOSE_THE_LOOP.md`). The plan below is retained for reference; remaining gap vs this spec: **`expected_impact_weekly` target column** in the change log + target-vs-actual check in the outcomes view (gated production change).
