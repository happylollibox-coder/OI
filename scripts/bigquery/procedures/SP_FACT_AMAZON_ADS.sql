...
BEGIN
  ...
  SELECT
    ...
    ads.sales - COALESCE(cost.TOTAL_COST_PER_UNIT, 0) * ads.units AS GROSS_PROFIT,
    ads.placement_type,
    ...
    ads.source_table,
    -- Derive ASIN from campaign_name for rows where most_advertised_asin_impressions is NULL
    CASE
      -- Lollibox variants
